...
def main(data_folder):
    ...
    for i, (frame_index, width, height, bin_filename) in enumerate(metadata):
        if i >= len(timestamps):
            print(f"⚠️ 跳过帧 {frame_index}，没有对应时间戳")
            continue

        bin_path = os.path.join(data_folder, bin_filename)
        if not os.path.exists(bin_path):
            print(f"⚠️ 未找到 bin 文件: {bin_path}")
            continue

        try:
            matrix_2d = process_bin_file(bin_path, width, height)
        except Exception as e:
            print(f"❌ 错误帧 {frame_index}: {e}")
            continue
...
if __name__ == "__main__":
    folder = input("请输入包含二进制数据和 metadata.csv 的文件夹路径：")
    main(folder)
